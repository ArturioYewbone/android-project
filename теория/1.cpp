...
#include <chrono>
#include <iomanip>
#include </home/user/Desktop/include/json.hpp> 
...
json executeSQLQuery(const std::string& query, bool isLogin) {
    PGconn *conn = PQconnectdb("user=mpi password=135a1 host=povt-cluster.tstu.tver.ru port=5432 dbname=iss");
    ...
    if (isLogin) {
        ...
        if (PQresultStatus(res) != PGRES_TUPLES_OK && PQresultStatus(res) != PGRES_COMMAND_OK) {
            ...
            if (error_msg.find("duplicate key value violates unique constraint") != std::string::npos) {
                return {{"status", "error"}, {"message", "This login is already occupied"}};
            }
        ...
        while ((valread = read(new_sock, buffer, 1024)) > 0) {
            ...
            try {
                ...
                std::cout << "[" << getCurrentTime() << "] Получено сообщение: " 
                          << "Тип команды: " << command_type << ", Команда: " << command << "\n";
                ...
                if (command_type == "sql"|| command_type == "sql_login") {
                    ...
                    response_json = executeSQLQuery(command, isLogin);
                } else {
